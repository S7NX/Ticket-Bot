...
module.exports = {
	data: new SlashCommandBuilder().setName('setup').setDescription('Setup Ticket Menu.').setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild),
	/**
	 *
...
async function CategorySelection(collector, guildSettings) {
	try {
            ...
            ...supportRoles,
				...
				for (let ticket of guildSettings.TicketMenuOptions) {
					...
					if (existingCategory) {
						...
						//find ticket index in guildsetting
						let index = guildSettings.TicketMenuOptions.findIndex((x) => x.value === ticket.value);
		...
		console.log('logchlCollector', logchlCollector);
	} catch (e) {
		console.log(e);
	}
}
async function logChannel(collector, guildSettings) {
	collector.on('collect', async (interaction) => {
		...
		if (guildSettings.PanelChannelID && guildSettings.PanelMessageID) {
			await interaction.update({  components:[],embeds:[new EmbedBuilder().setDescription('Ticket System Setup Complete. Panel would be Updated Soon')], ephemeral: true });
      let returned = await updatePanel(interaction, guildSettings);
      returned.panelmsg.edit({ embeds: [returned.embed], components: [returned.row] });
...
async function panelChannel(collector, guildSettings) {
    ...
    let returned = await updatePanel(interaction, guildSettings);
    let channel = interaction.channels.first()
		let panelmsg = channel.send({ embeds: [returned.embed], components: [returned.row] });
		guildSettings.PanelChannelID = interaction.values[0];
		guildSettings.PanelMessageID = panelmsg.id;
		await guildSettings.save();
			await interaction.update({  components:[],embeds:[new EmbedBuilder().setDescription('Ticket System Setup Complete.')], ephemeral: true });

	});
}
 async function updatePanel(interaction, guildSettings) {
  //fetches channel and message using guildSettings
  let panelChannel = await interaction.guild.channels.cache.get(guildSettings.PanelChannelID)
  let panelMessage = await panelChannel.messages.fetch(guildSettings.PanelMessageID)
